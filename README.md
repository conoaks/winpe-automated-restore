...
The repository includes both files that must be copied into the mounted WIM:

```text
auto_restore.cmd  -> Windows\System32\auto_restore.cmd
Winpeshl.ini      -> Windows\System32\Winpeshl.ini
```

`Winpeshl.ini` directs WinPE to launch the restore script automatically. Leave the existing `startnet.cmd` unchanged.

For automated building, run `Build-AutomatedRestoreWim.ps1` from PowerShell as Administrator:
