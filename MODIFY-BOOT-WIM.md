...
- The provided `auto_restore.cmd` file.
...
If no image is mounted at `C:\WimMount`, continue to Step 3.
...
Create `C:\WimMount` if it does not exist:
...
If the command displays files but DISM reports that no WIM is mounted there, do not blindly delete them. Confirm that `C:\WimMount` is the intended temporary directory first.
...
```powershell
Copy-Item `
    -LiteralPath "C:\WimMount\Windows\System32\auto_restore.cmd" `
    -Destination "C:\RecoveryWim\auto_restore-original.cmd" `
    -Force
```
...
Copy `auto_restore.cmd` into the mounted WIM and rename it to `auto_restore.cmd`:
...
Do not name the in-image copy `auto_restore.cmd` unless you also change the existing launcher. This project intentionally leaves the launcher unchanged.

Do not modify:

```text
C:\WimMount\Windows\System32\startnet.cmd
C:\WimMount\Windows\System32\Winpeshl.ini
```
...
```powershell
Copy-Item `
    -LiteralPath "C:\RecoveryWim\my-recovery.wim" `
    -Destination "V:\my-recovery.wim"
```
...
```powershell
New-Item -ItemType Directory -Path "C:\AlternateWimMount" -Force
```

Then substitute `C:\AlternateWimMount` for `C:\WimMount` in every command.
